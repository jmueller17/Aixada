...
require_once(__ROOT__ . "php/lib/order_mod.php");
...
try{
    ...
    switch (get_param('oper')) {
        ...
        print_stored_query('xml','get_orders_listing', get_param('from_date',''), get_param('to_date'), get_param('uf_id',0),0,get_param('limit',''));
    	...
    	case 'getOrderedProductsList':
    		print_stored_query('xml','get_ordered_products_list', get_param('order_id',0), get_param('provider_id',0), get_param('date',0) );
    		exit;
        ...
        case 'getOrderedProductsListPrices':
            $ord = new Order();
            $rs = $ord->get_ordered_products_with_prices(get_param_int('order_id'), get_param_int('provider_id'),get_param_date('date'));

         

            //desired output format
            $output_class = "output_format_xml";
    	...
    	case 'getProductQuantiesForUfs':
    		print_stored_query('xml','get_order_item_detail', get_param('order_id',0), get_param('uf_id',0), get_param('provider_id',0), get_param('date_for_order',0),0 );
    		exit;
    	...
    	//retrieves for a given provider- or product id, and date if order is open, closed, send-off... NOT USED... DELETE?!
  		case 'checkOrderStatus':
  			print_stored_query('xml','get_order_status', get_param('date',0), get_param('provider_id',0), get_param('product_id',0), get_param('order_id',0)  );
  			exit;
    	...
    	case 'checkValidationStatus':
    		print_stored_query('xml','get_validated_status', get_param('order_id',0), get_param('cart_id',0));
    		exit;
    	...
    	//retrieves info about originally ordered quanties and available items after received orders have been revised and distributed in carts. 
  		...
  		case 'orderDetailInfo':
  			print_stored_query('xml','get_detailed_order_info', get_param('order_id',0), get_param('provider_id'), get_param('date',0) );
  			exit;
